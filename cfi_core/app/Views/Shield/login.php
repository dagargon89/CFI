...
<head>
    ...
    <title><?= lang('Auth.login') ?> - CFI Logístico</title>
...
<body class="bg-surface text-on-surface font-body antialiased min-h-screen flex items-center justify-center relative selection:bg-primary selection:text-on-primary">
    ...
    <div class="w-full max-w-md bg-surface-container-low border border-outline-variant/30 shadow-2xl relative p-10 matte-grain">
        <div class="relative z-10">
            ...
            <h5 class="text-3xl font-headline font-bold text-center mb-2 tracking-tight">CFI Auth Gate</h5>
            ...
            <?php if (session('message') !== null) : ?>
                <div class="bg-primary/20 border-l-4 border-primary p-4 mb-8 text-primary text-sm font-medium">
                    <?= esc(session('message')) ?>
                </div>
            <?php endif ?>
            ...
            <form action="<?= url_to('login') ?>" method="post" class="space-y-6">
                ...
                <!-- Email / Username (Depending on validFields we use email or text for inputmode) -->
                <div>
                ...
                <div>
                    ...
                    <input type="password" class="w-full bg-surface-container-lowest border border-outline-variant/30 text-on-surface p-4 font-body focus:border-primary focus:outline-none transition-colors" id="floatingPasswordInput" name="password" inputmode="text" autocomplete="current-password" required>
                </div>

                <!-- Remember me -->
                <?php if (setting('Auth.sessionConfig')['allowRemembering']): ?>
                    <div class="flex items-center mt-4 pt-2">
                        <input type="checkbox" name="remember" class="w-4 h-4 bg-surface-container text-primary border-outline-variant/30 focus:ring-primary focus:ring-offset-surface cursor-pointer" <?php if (old('remember')): ?> checked<?php endif ?>>
                        <label class="ml-3 text-[10px] font-label uppercase text-on-surface-variant tracking-[0.1em] cursor-pointer font-bold">
                ...
                <div class="pt-6">
                    <button type="submit" class="w-full bg-primary hover:brightness-110 text-on-primary px-8 py-4 font-label text-[10px] uppercase tracking-[0.2em] font-bold transition-all flex justify-center items-center cursor-pointer shadow-[0_0_15px_rgba(209,161,78,0.3)] hover:shadow-[0_0_20px_rgba(209,161,78,0.5)]">
                        <?= lang('Auth.login') ?>
                        <span class="material-symbols-outlined ml-3 text-[18px]">login</span>
                    </button>
                </div>
                ...
                <?php if (setting('Auth.allowRegistration')) : ?>
                    <div class="mt-8 pt-6 border-t border-outline-variant/20 text-center">
                        <p class="text-[10px] font-label uppercase text-outline tracking-wider">
                            ...
                            <a href="<?= url_to('register') ?>" class="text-primary hover:text-white transition-colors ml-1 font-bold"><?= lang('Auth.register') ?></a>
                        </p>
                    </div>
                <?php endif ?>
            </form>
